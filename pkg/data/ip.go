...
type UniqueIP struct {
	IP          string           `bson:"ip"`
	NetworkUUID primitive.Binary `bson:"network_uuid"`
...
func NewUniqueIP(ip net.IP, agentUUID, agentName string) UniqueIP {
	...
	u.NetworkUUID = primitive.Binary{
...
func (u UniqueIP) Equal(ip UniqueIP) bool {
	return (u.IP == ip.IP &&
		u.NetworkUUID.Subtype == ip.NetworkUUID.Subtype &&
...
func (u UniqueIP) MapKey() string {
	...
	builder.WriteByte(u.NetworkUUID.Subtype)
...
type UniqueDstIP struct {
	...
	DstNetworkUUID primitive.Binary `bson:"dst_network_uuid"`
...
func (p UniqueIPPair) MapKey() string {
	...
	builder.WriteByte(p.DstNetworkUUID.Subtype)
